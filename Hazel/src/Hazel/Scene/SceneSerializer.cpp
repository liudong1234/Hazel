...
namespace YAML
{
    ...
    {
        static Node encode(const Hazel::UUID& rhs)
        {
            Node node;
            node.push_back(rhs);
            node.SetStyle(EmitterStyle::Flow);
        ...
        static bool decode(const Node& node, Hazel::UUID& rhs)
        {
            if (!node.IsSequence() || node.size() != 1)
                return false;
            rhs = node[0].as<Hazel::UUID>();
            return true;
        }
    };
}
...
namespace Hazel
{
    ...
    static void SerializeEntity(YAML::Emitter& out, Entity entity)
    {
        ...
        if (entity.HasComponent<ScriptComponent>())
        {
            ...
            Ref<ScriptClass> entityClass = ScriptEngine::GetEntityClass(tc.ClassName);
...
#define WRITE_FIELD_VALUE(FieldType, Type) \
            ...
            break
                    switch (field.Type)
                    {
                        case ScriptFieldType::Float:
                            ...
                            break;
                        WRITE_FIELD_VALUE(Double, double);
                        WRITE_FIELD_VALUE(Bool, bool);
                        WRITE_FIELD_VALUE(Byte, int8_t);
                        WRITE_FIELD_VALUE(Char, char);
                        WRITE_FIELD_VALUE(Short, int16_t);
                        WRITE_FIELD_VALUE(Int, int);
                        WRITE_FIELD_VALUE(Long, int64_t);
                        WRITE_FIELD_VALUE(UShort, uint16_t);
                        WRITE_FIELD_VALUE(UInt, uint32_t);
                        WRITE_FIELD_VALUE(ULong, uint64_t);
                        WRITE_FIELD_VALUE(Vector2, glm::vec2);
                        WRITE_FIELD_VALUE(Vector3, glm::vec3);
                        WRITE_FIELD_VALUE(Vector4, glm::vec4);
                        WRITE_FIELD_VALUE(Entity, UUID);
                        default:
                            break;
                    }
#undef FIELD_TYPE
        ...
        if (entity.HasComponent<SpriteRendererComponent>())
        {
            ...
            out << YAML::Key << "TillingFactor" << YAML::Value << sprite.TillingFactor;
            if (sprite.Texture)
                out << YAML::Key << "TexturePath" << YAML::Value << sprite.Texture->GetFilePath();
            out << YAML::EndMap;
        }
            ...
            for (auto entity : entities)
            {
                ...
                if (scriptComponent)
                {
                    ...
                    if (scriptFields)
                    {
                        ...
                        const std::map<std::string, ScriptField> fields = entityClass->GetFields();
                        auto& entityFields = ScriptEngine::GetScriptFieldMap(deserialedEntity);


                        for (auto sf : scriptFields)
                        {
                            std::string fieldName = sf["FieldName"].as<std::string>();
                            HZ_CORE_ASSERT(fields.find(fieldName) != fields.end());
                            if (fields.find(fieldName) == fields.end())
                                continue;
                            ScriptFieldType type = Utils::ScriptFieldTypeFromString(sf["Type"].as<std::string>());
                            
                            //ScriptField field = fields.at(name);
                            ScriptFieldInstance si;
                            si.Field = fields.at(fieldName);
#define READ_FIELD_VALUE(FieldType, Type) \
    case ScriptFieldType::FieldType:      \
    {                                     \
        Type data = sf["Data"].as<Type>();\
        si.SetValue<Type>(data);          \
        break;                            \
    }                                     
                            switch (type)
                            {
                                case Hazel::ScriptFieldType::None: break;
                                case Hazel::ScriptFieldType::Float:
                                {
                                    float data = sf["Data"].as<float>();
                                    si.SetValue<float>(data);
                                    break;
                                }
                                READ_FIELD_VALUE(Double, double);
                                READ_FIELD_VALUE(Bool, bool);
                                READ_FIELD_VALUE(Byte, int8_t);
                                READ_FIELD_VALUE(Char, char);
                                READ_FIELD_VALUE(Short, int16_t);
                                READ_FIELD_VALUE(Int, int);
                                READ_FIELD_VALUE(Long, int64_t);
                                READ_FIELD_VALUE(UShort, uint16_t);
                                READ_FIELD_VALUE(UInt, uint32_t);
                                READ_FIELD_VALUE(ULong, uint64_t);
                                READ_FIELD_VALUE(Vector2, glm::vec2);
							    READ_FIELD_VALUE(Vector3, glm::vec3);
                                READ_FIELD_VALUE(Vector4, glm::vec4);
                                READ_FIELD_VALUE(Entity, UUID);
                                default: break;
                            }
#undef TYPE_WITH_VALUE
                            entityFields[fieldName] = si;
                        }
                ...
                if (spriteComponent)
                {
                    ...
                    if (spriteComponent["TillingFactor"])
                        sprite.TillingFactor = spriteComponent["TillingFactor"].as<float>();
                }
